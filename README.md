...
- **Propiedades / Naves Industriales / Terrenos**: tres apartados separados en el menú público, cada uno con su propio listado y filtros — `/propiedades` (casas y departamentos), `/naves-industriales` y `/terrenos`. Los tres reutilizan el mismo componente (`src/pages/Properties.jsx`, parametrizado por `allowedTypes`/`titleKey`/`subtitleKey`) y el mismo filtro (`PropertyFilters.jsx`); en Naves Industriales y Terrenos el filtro de Tipo se oculta porque ya está implícito en la sección. Los filtros incluyen tipo (en `/propiedades`), **operación** (En venta / Se busca comprar — `properties.operation_type`, nuevo), zona, precio y superficie. Vista de mapa interactivo con Leaflet (marcadores por tipo, popup con foto/precio/superficie), y vista de detalle compartida (`/propiedades/:id`, para cualquier tipo) con galería, características, ubicación y tarjeta del asesor (teléfono, correo, WhatsApp). La calculadora de valor estimado (`/calculadora`) sigue existiendo pero ya no aparece en el menú — solo es accesible por URL directa.
- **Simulador de crédito Infonavit** (`/admin/credito-infonavit`, interno): a partir de edad, sexo, salario mensual y saldo en la Subcuenta de Vivienda, estima el crédito tradicional disponible (fórmula estándar de anualidad sobre la cuota mensual que resulta de aplicar un factor de descuento de referencia al salario), la cuota mensual, el total disponible para compra y un desglose de gastos/plazo (`src/lib/infonavitSimulator.js`, función pura). Pensado para que el asesor lo use en pantalla con el cliente, no como página pública del sitio. Los parámetros oficiales (UMA, tasas, regla de edad) se investigaron por búsqueda web en 2026-09 y están fijos en el código con su fuente citada en la propia página — no hay API pública de Infonavit para consultarlos en vivo, así que hay que actualizarlos a mano cuando cambien (la UMA se revisa cada febrero).

  ⚠️ **Limitación conocida sobre las tasas de interés**: el PDF oficial de Infonavit con la tabla escalonada completa de tasas por nivel salarial (portalmx.infonavit.org.mx) fue inalcanzable (timeout) al construir esta herramienta, por varios métodos de acceso. Los dos extremos de la tabla sí están confirmados por varias fuentes coincidentes (3.69% hasta 2.6 UMA, 10.45% desde 6.6 UMA); los tramos intermedios se interpolan linealmente entre esos dos puntos como aproximación — **no es la tabla oficial real**, que es escalonada. La página lo declara explícitamente en su propio aviso, además de aclarar (como pide toda herramienta de este tipo) que el resultado es una estimación conceptual previa a la consulta oficial en "Mi Cuenta Infonavit". Si en algún momento se logra acceder al PDF oficial, hay que reemplazar `tasaInteresPorUma()` en `src/lib/infonavitSimulator.js` por la tabla escalonada real.
...
- **Liquidaciones** (botón "Liquidación" en cada fila de `/admin/propiedades`, **confidencial**): devolución al vendedor original, inversión (remodelación + servicios), comisión de captación/venta y utilidad neta de la sociedad por vivienda, calculados en cascada (`src/lib/liquidacion.js`) a partir de montos y porcentajes capturables (no fijos en el código). A diferencia de **todas** las demás tablas de este proyecto (RLS = "cualquier usuario autenticado"), `liquidaciones` está restringida por RLS a los dos correos de los socios (`src/lib/partners.js`, política en `supabase/schema.sql`) — ni el botón ni la ruta se muestran a nadie más (`PartnerRoute.jsx`), pero la restricción real vive en la base de datos: un tercer usuario autenticado que intentara leer la tabla directo por la API de Supabase recibiría 0 filas, no solo una interfaz oculta.
  ...
  **Costo total e inversión en remodelación no se capturan aquí.** El "Costo total de liquidación" se toma en vivo del precio registrado en Propiedades, y la "Inversión — costo de remodelación" se toma en vivo del total de materiales (precio de ferretería propia) del proyecto de remodelación vinculado a esa casa. Ninguno de los dos se guarda como cifra fija en `liquidaciones` — se recalculan solos cada vez que se abre o se guarda la pantalla, así que si el precio cambia en Propiedades o se agregan/editan materiales en Remodelaciones, la liquidación lo refleja de inmediato sin pedir confirmación. Solo "Pago de servicios" sigue siendo un monto capturado a mano en este módulo.
