...
- **Simulador de crédito Infonavit** (`/credito-infonavit`, público): a partir de edad, sexo, salario mensual y saldo en la Subcuenta de Vivienda, estima el crédito tradicional disponible (fórmula estándar de anualidad sobre la cuota mensual que resulta de aplicar un factor de descuento de referencia al salario), la cuota mensual, el total disponible para compra y un desglose de gastos/plazo (`src/lib/infonavitSimulator.js`, función pura). Los parámetros oficiales (UMA, tasas, regla de edad) se investigaron por búsqueda web en 2026-09 y están fijos en el código con su fuente citada en la propia página — no hay API pública de Infonavit para consultarlos en vivo, así que hay que actualizarlos a mano cuando cambien (la UMA se revisa cada febrero).
