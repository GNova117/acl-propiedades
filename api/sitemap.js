...
export default async function handler(req, res) {
  ...
  let debug = `env: url=${Boolean(url)} key=${Boolean(key)}`;
  if (url && key) {
    try {
      const supabase = createClient(url, key);
      const { data, error } = await supabase.from("properties").select("id, updated_at").eq("active", true);
      debug += ` | rows=${data ? data.length : "null"} error=${error ? error.message : "none"}`;
      ...
      debug += ` | threw=${err instanceof Error ? err.message : String(err)}`;
    }
  }
